...
**
** $Id: si3226_intf.c 862 2009-06-18 23:44:35Z cdp@SILABS.COM $
**
** SI3226_Intf.c
...
uInt32 irt_ac = 70;   /* AC Ringtrip current in mApk */
uInt32 irt_dc = 10;   /* DC Ringtrip current in mA */
...
const ProSLIC_SineAcRingtripLookup sineAcRingtripTable[] =
/*  Freq rtacth rtacdb*/
   { {20, 8425891L, 24576L},
    {25, 6740713L, 32768L},
    {30, 5617260L, 32768L},
...
const ProSLIC_SineDcRingtripLookup sineDcRingtripTable[] =
/*  Freq rtdcth rtdcdb*/
   { {20, 1864135L, 24576L},
    {25, 1491308L, 32768L},
    {30, 1258291L, 32768L},
...
const ProSLIC_TrapAcRingtripLookup trapAcRingtripTable[] =
/*  Freq rtacthCR12 rtacthCR13 rtacthCR14 rtacthCR15 */
    {{20, {10728486L,  10261705L, 9891329L,  9592529L},24576L},
    {25, {8582789L,  8209364L,  7913063L,  7674023L},  32768L},
    {30, {7152324L,  6841137L,  6594219L,  6395019L},  32768L},
    {35, {12261127L, 11727663L, 11304376L, 10962890L}, 24576L},
    {40, {10728486L, 10261705L, 9891329L,  9592529L},  24576L},
    {45, {9536432L,  9121516L,  8792293L,  8526692L},  24576L},
    {50, {8582789L,  8209364L,  7913063L,  7674023L},  32768L},
    {0,{0L,0L,0L,0L},0}}; /* terminator */

const ProSLIC_TrapDcRingtripLookup trapDcRingtripTable[] =
/*  Freq rtacthCR12 rtacthCR13 rtacthCR14 rtacthCR15 */
    {{20, {10728486L,  10261705L, 9891329L,  9592529L},24576L},
    {25, {8582789L,  8209364L,  7913063L,  7674023L},  32768L},
    {30, {7152324L,  6841137L,  6594219L,  6395019L},  32768L},
    ...
    switch(ringCfg->ringtype)
    {
        case ProSLIC_RING_SINE:
            ...
            /* Setup ringtrip - calculate both and select which one is more appropriate later */
            /* AC ringtrip fixed at 70mA, DC ringtrip fixed at 10mA */
            if(sineRingFreqTable[i].freq >32)
            {
                Si3226_Ring_Presets[preset].rtper = (6553600L*2)/sineRingFreqTable[i].freq;
            }
            else
            {
                Si3226_Ring_Presets[preset].rtper = 6553600L/sineRingFreqTable[i].freq;
            }

            Si3226_Ring_Presets[preset].rtacdb = sineAcRingtripTable[i].rtacdb;
            Si3226_Ring_Presets[preset].rtdcdb = sineDcRingtripTable[i].rtdcdb;

            /* If DC Offset >= 10v, use DC Ringtrip, otherwise, use AC Ringtrip */
            /* Also, do not program offset if outside of this range */
            if((ringCfg->offset >= 10)&&(ringCfg->offset <=50))
            {
               Si3226_Ring_Presets[preset].rtacth = 0xFFFFFFFL;
               Si3226_Ring_Presets[preset].rtdcth = sineDcRingtripTable[i].rtdcth;
               Si3226_Ring_Presets[preset].offset = ringCfg->offset * vScale;
            }
            else
            {
               Si3226_Ring_Presets[preset].rtdcth = 0xFFFFFFFL;
               Si3226_Ring_Presets[preset].rtacth = sineAcRingtripTable[i].rtacth;
               Si3226_Ring_Presets[preset].offset = 0L;
            }
        ...
        case ProSLIC_RING_TRAP_CF15:
            ...
            {
                Si3226_Ring_Presets[preset].rtper = (6553600L*2)/trapRingFreqTable[i].freq;
            }
            else
            {
                Si3226_Ring_Presets[preset].rtper = 6553600L/trapRingFreqTable[i].freq;
            }
            Si3226_Ring_Presets[preset].rtacdb = trapAcRingtripTable[i].rtacdb;
            Si3226_Ring_Presets[preset].rtdcdb = trapDcRingtripTable[i].rtdcdb;
            ...
            {
               Si3226_Ring_Presets[preset].rtacth = 0xFFFFFFFL;
               Si3226_Ring_Presets[preset].rtdcth = trapDcRingtripTable[i].rtdcth[ringCfg->ringtype];
               Si3226_Ring_Presets[preset].offset = ringCfg->offset * vScale;
            }
            else
            {
               Si3226_Ring_Presets[preset].rtdcth = 0xFFFFFFFL;
               Si3226_Ring_Presets[preset].rtacth = trapAcRingtripTable[i].rtacth[ringCfg->ringtype];
               Si3226_Ring_Presets[preset].offset = 0L;
            }
