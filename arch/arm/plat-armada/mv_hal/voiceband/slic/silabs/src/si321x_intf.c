...
**
** $Id: si321x_intf.c 561 2009-03-26 14:43:33Z cdp@SILABS.COM $
**
** Proslic.c
**
** Author(s):
	...
	if (pProslic[0]->bomOption == SI321X_HV) { /*reprogram battery voltages */
		for (j=0;j<size;j++){
			if (pProslic[j]->channelEnable){ /*make sure channel didn't fail foreign voltage check
				pProslic[j]->WriteRegX(pProslic[j]->pProHWX, pProslic[j]->channel, 74, 0x1d);/*vbath */
				pProslic[j]->WriteRegX(pProslic[j]->pProHWX, pProslic[j]->channel, 75, 0x9);/*vbatl */
				pProslic[j]->WriteRegX(pProslic[j]->pProHWX, pProslic[j]->channel, 72, 0x12);/*voc */
				pProslic[j]->WriteRegX(pProslic[j]->pProHWX, pProslic[j]->channel, 73, 0x1);/*vcm */
			}
		}
	}
...
int Si321x_SetMuteStatus (proslicChanType_ptr pProslic, ProslicMuteModes muteEn){
	...
	if (muteEn & PROSLIC_MUTE_RX){
		WriteReg (pProHW,pProslic->channel,AUDIO_LOOPBACK,regTemp|(1<<4));
	}
	if (muteEn & PROSLIC_MUTE_TX){
		WriteReg (pProHW,pProslic->channel,AUDIO_LOOPBACK,regTemp|(1<<5));
	}
	return 0;
}
...
int Si321x_SetLinefeedStatusBroadcast (proslicChanType *pProslic,uInt8 newLinefeed){
	WriteReg (pProHW, BROADCAST, LINE_STATE,newLinefeed);
	return 0;
}
